...
int main(){
    int a_xor,b_xor,a1,a2,s_a1,s_a2,gamma,k[8][64];
    for (int i = 0; i < 8; ++i){
      for(int j = 0 ; j < 64 ; ++j)
        k[i][j] = 0;
    }
    FILE *f_b_xor,*f_g_xor,*fo,*f_a;
    f_a = fopen("alphafirst.txt","r+");
    f_b_xor = fopen("bxor.txt","r+");
    f_g_xor = fopen("yxor.txt","r+");
    fo = fopen("frekey.txt","w+");
    long int cont = 0;
    ...
    while(count <= 65430)  {
      ...
      for(int j = 0 ; j < 8 ; j++){
        ...
        for (int i = 0; i < 64; ++i)
        {
          ...
          if((s_a2^s_a1) == temp_b) k[j][a1^temp_g] += 1;
        }
    ...
    { 
      ...
      for(int j = 0 ; j < 64 ; ++j){
        ...
        if (m<k[i][j]){ m=k[i][j];
        A[i]=j;}}
      fprintf(fo, "---------------Max Frequency key = %d ------------------\n",A[i]);
    }
    return 0;
}
