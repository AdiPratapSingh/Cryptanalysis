...
void key_as_bits(INT sw1, BYTE *CD, INT r) //INT sw1;
// BYTE *CD;
// INT r;
{
    register INT i, j, k, t1, t2;
...
void unpack8(BYTE *packed, BYTE *binary)// BYTE *packed;
//BYTE *binary;
{
    register INT i, j, k;
...
void pack8(BYTE *packed, BYTE *binary) // BYTE *packed;
// BYTE *binary;
{
    register INT i, j, k;
...
void des(BYTE *in, BYTE *out, INT r, char flag) //BYTE *in; /* packed 64 bit Input block */ 
...
void tobyte(char in[], BYTE inp[]) {
    ...
    for (i = 0; 2 * i < strlen(in); ++i) {
        inp[i] = ((int)(in[2 * i] - 'd')) * 16 + (in[2 * i + 1] - 'd');
    }
}
...
void tostr(BYTE out[]) {
    ...
    for (i = 0; i < 8; ++i) {
        off = out[i];
        printf("%c%c", (char)('d' + (off / 16)), (char)('d' + (off % 16)));
    }
    printf("\n");
}
...
int main(int argc, char **argv) {
    ...
    BYTE key[57] = "01101110010111100111101110001100101100001000001101110001";
    BYTE KEY[57];
    for (int i = 0; i < 56; i++) {
        KEY[i] = key[i]-'0';
    }
    
    char password[] = "sqknijrmjfgoslfknkhrfsqhsihgioqm";
    BYTE lefthalf[16], righthalf[16];

    // Since cyphertext is of char long
    // Passed as two epoch
    for (int i = 0; i < 16; i++) {
        lefthalf[i] = password[i];
        righthalf[i] = password[16+i];
    }
    BYTE leftbyte[100], rightbyte[100];
    tobyte((char *)lefthalf, leftbyte);
    tobyte((char *)righthalf, rightbyte);

    BYTE loutput[100], routput[100];

    key_as_bits(1, KEY, 6);
    des(leftbyte, loutput, 6, 'N');
    des(rightbyte, routput, 6, 'N');
